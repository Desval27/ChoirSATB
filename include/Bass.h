...
{
public:
    TheBass(const MyTimeSignature &ts,
            const MyTuningReference &tr,
            const MyTemperament &t,
            const MyScaleMap &s)
        // -3 Relative to C4 = C1
        : TheVoice(ts, tr, t, s, -3, 0.3, 0.1, 0.7, 0.2, Music::SCALE_WEIGHTS_7_TONIC_HEAVY)
    {
    }
                              ...
                              MyChordEventSet &chords) override
    {
        ...
        for (size_t i = 0; i < maxSize; i++)
        {
            ...
            case 5:
                ...
                len1 = max(ts.beatValue, static_cast<Music::NoteValue>(chords[i].value - len2));
                events.Emplace(n, 0, len1);
