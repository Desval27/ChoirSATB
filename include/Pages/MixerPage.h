...
protected:
  void InternalDraw(daisy::OneBitGraphicsDisplay &display,
    ...
    display.WriteStringAligned("MIXER", TitleFont, titleRect,
    ...
    for (int i = 0; i < theApp.NUM_VOICES; i++) {
      ...
      Rectangle voiceRect((i * voiceWidth), titleRect.GetBottom(), voiceWidth,
                          voiceHeight);
      ...
      Rectangle volumeRect((i * voiceWidth),
                           voiceRect.GetBottom() + (fullBarHeight - barHeight),
                           voiceWidth, barHeight);
