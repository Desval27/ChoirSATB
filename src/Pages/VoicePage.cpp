...
bool
VoicePage::OnArrowButton(ArrowButtonType arrowType,
...
bool
VoicePage::OnMenuEncoderTurned(int16_t turns, uint16_t stepsPerRevolution)
{
  ...
  //   if (turns > 0) {
  //     v->SetWaveform(
  //       wrap(v->GetWaveform() + 1, static_cast<int>(Oscillator::WAVE_LAST) -
  //       1));
  //   } else if (turns < 0) {
  //     v->SetWaveform(
  //       wrap(v->GetWaveform() - 1, static_cast<int>(Oscillator::WAVE_LAST) -
...
bool
VoicePage::OnPotMoved(uint16_t potID, float newPosition)
{
  //   TheApp& theApp = TheApp::instance();
  //   TheVoice* v = theApp.GetVoice(_voiceIndex);
...
bool
VoicePage::OnCancelButton(uint8_t numberOfPresses, bool isRetriggering)
{
  Close();
  return true;
}
...
void
VoicePage::Draw(const UiCanvasDescriptor& canvas)
{
...
// void
// VoicePage::DrawADSR(MyDisplay* d, const Rectangle& rect, TheVoice* v)
// {
...
//   const int x3 = x2 + segW;                           // End of Sustain
//   (held) const int x4 = x3 + (v->GetReleaseAs0to1() * segW); // End of
//   Release
